defmodule KanbanVisionApi.Usecase.Boards.ReorderWorkflowStep do
  @moduledoc """
  Use Case: Reorder a workflow step inside a board.
  """
  ...
  @spec execute(
          ReorderBoardWorkflowStepCommand.t(),
          BoardRepository.repository_runtime(),
          keyword()
        ) ::
          result()
  def execute(%ReorderBoardWorkflowStepCommand{} = cmd, repository_runtime, opts \\ []) do
    correlation_id = Keyword.get(opts, :correlation_id, UUID.uuid4())
    repository = RepositoryConfig.fetch_from_opts!(__MODULE__, opts)

    Logger.info("Reordering workflow step in board",
    ...
    with {:ok, board} <- repository.get_by_id(repository_runtime, cmd.board_id),
         {:ok, updated_board} <- Board.reorder_workflow_step(board, cmd.step_id, cmd.order),
         {:ok, persisted_board} <- repository.update(repository_runtime, updated_board) do
      Logger.info("Workflow step reordered successfully",
        correlation_id: correlation_id,
      ...
      EventEmitter.emit(
        :board,
        :board_workflow_step_reordered,
        %{board_id: persisted_board.id, step_id: cmd.step_id, step_order: cmd.order},
        correlation_id
      )
      ...
      {:error, :step_not_found} ->
        ...
        log_error("Failed to reorder workflow step", cmd, correlation_id, error)
        error
