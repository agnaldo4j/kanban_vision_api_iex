defmodule KanbanVisionApi.Agent.OrganizationsTest do
  ...
  describe "When start the system with empty state" do
    ...
    test "should be able to add new organization", %{actor_pid: pid, organizations: _organizations} = _context do
      ...
      assert KanbanVisionApi.Agent.Organizations.add(pid, domain) == :ok
    end

    @tag :domain_organizations
    test "should be able to delete an existent organization", %{actor_pid: pid, organizations: _organizations} = _context do
      domain = KanbanVisionApi.Domain.Organization.new("ExampleOrg")
      assert KanbanVisionApi.Agent.Organizations.add(pid, domain) == :ok
    ...
    @tag :domain_organizations
    test "should not be able to delete a non-existent organization", %{actor_pid: pid, organizations: _organizations} = _context do
  ...
  describe "When start the system with one organization at the state" do
    ...
    test "should has one organization", %{actor_pid: pid, organizations: organizations, domain: my_domain} = _context do
      template = %KanbanVisionApi.Agent.Organizations{id: organizations.id, organizations: %{my_domain.id => my_domain}}
      assert KanbanVisionApi.Agent.Organizations.get_all(pid) == template
    end
